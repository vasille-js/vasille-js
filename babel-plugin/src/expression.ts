...
function addExternalIValue(path: NodePath<types.MemberExpression | types.OptionalMemberExpression>, search: Search) {
    ...
    if (!search.found.has(name)) {
        search.found.set(
            name,
            t.callExpression(t.memberExpression(search.external.id, t.identifier("r$")), [path.node.object]),
        );
    }
    path.replaceWith(encodeName(name));
}
    ...
    switch (expr.type) {
        ...
        case "OptionalMemberExpression": {
            ...
            if (
                ...
                search.external.stack.get(node.object.name) === VariableState.ReactiveObject
            ) {
